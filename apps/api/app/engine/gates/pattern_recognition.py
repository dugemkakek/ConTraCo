...
Fix: walrus operator bug on line ~97 was reassigning `signals` variable
to `signs` (a list[int]), which broke both confidence calculation and
the sub_signals evidence dict comprehension below it.
"""
...
class PatternRecognitionGate:
    ...
    async def evaluate(self, ctx: GateContext) -> GateEvaluation:
        ...
        peak_indices = [
            i for i in range(1, len(recent_highs) - 1)
            if recent_highs[i] > recent_highs[i - 1] and recent_highs[i] > recent_highs[i + 1]
        ]
        if len(peak_indices) >= 2:
            p1, p2 = peak_indices[-2], peak_indices[-1]
            if abs(recent_highs[p1] - recent_highs[p2]) / max(recent_highs[p1], 1e-9) < 0.01:
                valley = min(recent_lows[p1:p2 + 1]) if p2 > p1 else 0.0
                if valley < recent_highs[p1] * 0.98:
                    patterns.append(("double_top", -0.8))

        # Double bottom
        trough_indices = [
            i for i in range(1, len(recent_lows) - 1)
        ...
        if len(trough_indices) >= 2:
            ...
            if abs(recent_lows[t1] - recent_lows[t2]) / max(recent_lows[t1], 1e-9) < 0.01:
                peak_between = max(recent_highs[t1:t2 + 1]) if t2 > t1 else 0.0
        ...
        if n >= 20:
            ...
            pole_pct = pole / closes[-30] * 100 if n >= 30 and closes[-30] != 0 else 0.0
            flag_range = max(highs[-10:]) - min(lows[-10:])
            flag_pct = flag_range / closes[-1] * 100 if closes[-1] != 0 else 999.0
        ...
        # FIX: was using walrus `:=` which reassigned `signals` to list[int],
        # breaking sub_signals dict below. Now uses clean separate variable.
        signs = [1 if s > 0 else -1 if s < 0 else 0 for _, s in patterns]
        pos = sum(1 for x in signs if x > 0)
        neg = sum(1 for x in signs if x < 0)
        confidence = max(pos, neg) / len(signs) if signs else 0.0
